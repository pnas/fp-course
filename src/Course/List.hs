...
filter ::
  ...
  -> List a
filter f Nil = Nil
filter f ( a :. b ) = case f a of True -> a :. filter f b
                                  False -> filter f b
...
flattenAgain ( h :. t ) = flatMap (\x -> x) ( h :. t)
...
lengthGT4 ::
  ...
  -> Bool
lengthGT4 Nil = False
lengthGT4 t = if length(t) > 4 then True else False
